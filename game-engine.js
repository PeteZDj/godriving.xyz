/* ============================================================
 *  GoDriving.xyz — shared game engine
 *  Loaded by every lesson HTML page. Exposes a global GoDriving
 *  object with init/setMission/start/onUpdate/onRender hooks.
 * ============================================================ */
(function (global) {
'use strict';
...
const _rng = mulberry32(7);

const PARK_ROAD = {
  x: 0, yTop: HW_SOUTH_Y, yBot: PARK.y - 20,
  laneWidth: LANE_WIDTH, lanes: 1,
};
...
function onHighway(x, y, m = 0) {
  return Math.abs(x - HW_CENTER_X) < HW_HALF + m
      && y > HW_NORTH_Y - m && y < HW_SOUTH_Y + m;
}
function onParkRoad(x, y, m = 0) {
  return Math.abs(x - PARK_ROAD.x) < LANE_WIDTH + m
      && y > PARK_ROAD.yTop - m && y < PARK_ROAD.yBot + m;
}
function onParkLot(x, y, m = 0) {
  return x > PARK.x - m && x < PARK.x + PARK.w + m
      && y > PARK.y - m && y < PARK.y + PARK.h + m;
}
...
function onAnyRoad(x, y, m = 0) {
  return onHighway(x, y, m) || onParkRoad(x, y, m)
      || onParkLot(x, y, m) || onRoundabout(x, y, m)
      || onSideRoad(x, y, m);
}
...
const audio = {
  ...
  complete() {
    [523.25, 659.25, 783.99, 1046.5].forEach((f, i) =>
      setTimeout(() => this.tone(f, 0.18 + i*0.05, 'triangle', 0.28), i * 110));
  },
  fail() {
    this.tone(220, 0.28, 'sawtooth', 0.22);
    setTimeout(() => this.tone(165, 0.36, 'sawtooth', 0.22), 160);
  },
  crash() {
  ...
  startMusic() {
    ...
    const chords = [
      ...
      { root: 110.00, intervals: [0, 3, 7, 12] },
      { root:  87.31, intervals: [0, 4, 7, 12] },
      { root:  98.00, intervals: [0, 4, 7, 12] },
    ];
...
function worldToScreen(x, y) {
  return [
    (x - camera.x) * camera.zoom + innerWidth  / 2,
    (y - camera.y) * camera.zoom + innerHeight / 2,
  ];
}
...
function drawBackground() {
  ...
  const tile = 220;
  const startX = Math.floor((camera.x - innerWidth  / camera.zoom) / tile) * tile;
  const startY = Math.floor((camera.y - innerHeight / camera.zoom) / tile) * tile;
  for (let x = startX; x < camera.x + innerWidth  / camera.zoom; x += tile) {
    for (let y = startY; y < camera.y + innerHeight / camera.zoom; y += tile) {
      const [sx, sy] = worldToScreen(x, y);
...
function drawBuildings() {
  for (const b of BUILDINGS) {
    ...
    if (sx + b.w * camera.zoom < -50 || sx > innerWidth + 50 ||
        sy + b.h * camera.zoom < -50 || sy > innerHeight + 50) continue;
...
function drawNextMarkerIndicator(markers) {
  ...
  const margin = 80;
  if (sx >= margin && sx <= innerWidth - margin && sy >= margin && sy <= innerHeight - margin) return;
  const dx = sx - innerWidth / 2;
  const dy = sy - innerHeight / 2;
  const ang = Math.atan2(dy, dx);
  const r = Math.min(innerWidth, innerHeight) / 2 - 100;
...
function updateHud() {
  ...
  if (sv) sv.textContent = speedKmh;
  let gear = 'N';
  if (car.speed > 5) gear = 'D';
  else if (car.speed < -5) gear = 'R';
  if (car.handbrake) gear = 'P';
  const gv = document.getElementById('gear-value');
  if (gv) gv.textContent = gear;
  const sl = document.getElementById('signal-left');
  const sr = document.getElementById('signal-right');
  const blink = Math.floor(performance.now() / 250) % 2 === 0;
  if (sl) sl.classList.toggle('active', car.signalLeft && blink);
  if (sr) sr.classList.toggle('active', car.signalRight && blink);
}
...
function tickSignals() {
  ...
  if (phase !== _lastBlink && (car.signalLeft || car.signalRight)) {
    audio.tone(phase ? 1700 : 900, 0.06, 'square', 0.10);
  }
  _lastBlink = phase;
}
...
let updateHook = null;
let drawObjectsHook = null; // draws extra world-space objects (cones, etc.)
...
const GoDriving = {
  ...
  /**
   * init({ start, zoom, parkedSpots, traffic })
   *   - start: { x, y, heading } initial player position
   *   - zoom: initial camera zoom (camera.lockedZoom)
   *   - parkedSpots: list of indexes to populate with parked cars (or null = none)
   *   - traffic: opts for spawnTraffic (or false to skip)
   */
  init(opts = {}) {
    canvas = document.getElementById('game');
    ctx = canvas.getContext('2d');
    mini = document.getElementById('minimap-canvas');
    ...
    setupInput({ onReset: opts.onReset });

    // Wire mute button if present (HUD inserts one for us)
    const hud = document.getElementById('hud');
    if (hud && !document.getElementById('mute-btn')) {
  ...
  setMission(m) {
    ...
    if (stepEl && m.step) stepEl.innerHTML = m.step;
    const pb = document.getElementById('mission-progress');
  ...
  resetCar() {
    if (this._initialStart) {
      this.setStart(this._initialStart.x, this._initialStart.y, this._initialStart.heading);
    }
  },
  rememberStart() {
    this._initialStart = { x: car.x, y: car.y, heading: car.heading };
  },
  ...
  start() {
    ...
    const frame = (now) => {
      ...
      if (mission && mission.markers) {
        ...
        const pb = document.getElementById('mission-progress');
        if (pb && total > 0) pb.style.width = (hits / total * 100) + '%';
        if (mission.check ? mission.check() : (total > 0 && hits === total)) {
          if (mission.onComplete) mission.onComplete();
          GoDriving._mission = null; // ownership returns to lesson
        }
      }
      // RENDER
